...
######## JHU state
jhu_states <- load_jhu(level='State',pull_date = '2020-05-12', start_date = '2020-03-12', end_date = '2020-05-04')
...
######## Covidtracking state
covidtracking_states <- load_covid_tracking(level='State',pull_date = '0512', start_date = '2020-03-12', end_date = '2020-05-04')
non_us_stateName <- unique(covidtracking_states$stateName)[c(4,13,28,43,51)] # non US states
covidtracking_states <- covidtracking_states[covidtracking_states$stateName %in% setdiff(unique(covidtracking_states$stateName), non_us_stateName),]
covidtracking_states[covidtracking_states$stateName == "District Of Columbia","stateName"] <- "District of Columbia"

######## nyt state
## placeholder

######## yu state
yu_states <- load_yu(level='State',pull_date = '05_05_2020', start_date = '2020-03-12', end_date = '2020-05-04')
...
for (stateName in unique(jhu_states$stateName)){
  ...
  state_data_covidtracking <- covidtracking_states[covidtracking_states$stateName == stateName,]
  ## placeholder for nyt
  state_data_yu <- yu_states[yu_states$stateName == stateName,]
  
  dates_list <- list(state_data_jhu$date, state_data_covidtracking$date, state_data_yu$date)
  positive_increase_list <- list(state_data_jhu$positiveIncrease, state_data_covidtracking$positiveIncrease, state_data_yu$positiveIncrease)
  state_result <- sensitivity_data_EpiEstim(dates_list = dates_list,
                                            positive_increase_list = positive_increase_list
                                            )
  png(paste0("./COVID19-Rt/sensitivity_analysis/results_4sources_state/state_rt/estimate_R_",stateName,".png"), width=8, height=6, units = 'in', res = 300)
  print(estimate_R_plots(state_result, what = "R",
                         options_R = list(col = c("red", "orange", "blue")), legend = TRUE))
  dev.off()
}


######## 4 sources state sensitivity analysis
state_Rt_EpiEstim_jhu <- estimate_rt_EpiEstim_state(state_data_combined = jhu_states,
                                                        mean_serial = 5.2,
                                                        std_serial = 5.1)

state_Rt_EpiEstim_covidtracking <- estimate_rt_EpiEstim_state(state_data_combined = covidtracking_states,
                                                        mean_serial = 5.2,
                                                        std_serial = 5.1)
...
Rt_Apr1_EpiEstim_covidtracking <- as.numeric(as.character(unlist(lapply(state_Rt_EpiEstim_covidtracking,function(x){x[x$interval_end=="2020-04-01","mean_rt"]}))))
...
data_for_heatmap_Apr1 <- cbind(jhu = Rt_Apr1_EpiEstim_jhu,
                               covidtracking = Rt_Apr1_EpiEstim_covidtracking,
                               yu = Rt_Apr1_EpiEstim_yu)

M1 <- cor(data_for_heatmap_Apr1)
M1[which(M1 > 1)] <- 1
png("./COVID19-Rt/sensitivity_analysis/results_4sources_state/corr_heatmap_Apr1_4sources.png", width = 8, height = 8, units = 'in', res = 300)
corrplot(M1,
         method = "circle",
         tl.col = "black",
         type = "upper",
...
png("./COVID19-Rt/sensitivity_analysis/results_4sources_state/slope_heatmap_Apr1_4sources.png", width = 8, height = 8, units = 'in', res = 300)
...
Rt_Apr24_EpiEstim_covidtracking <- as.numeric(as.character(unlist(lapply(state_Rt_EpiEstim_covidtracking,function(x){x[x$interval_end=="2020-04-24","mean_rt"]}))))
...
data_for_heatmap_Apr24 <- cbind(jhu = Rt_Apr24_EpiEstim_jhu,
                               covidtracking = Rt_Apr24_EpiEstim_covidtracking,
                               yu = Rt_Apr24_EpiEstim_yu)

M1 <- cor(data_for_heatmap_Apr24)
M1[which(M1 > 1)] <- 1
png("./COVID19-Rt/sensitivity_analysis/results_4sources_state/corr_heatmap_Apr24_4sources.png", width = 8, height = 8, units = 'in', res = 300)
corrplot(M1,
         method = "circle",
         tl.col = "black",
         type = "upper",
...
png("./COVID19-Rt/sensitivity_analysis/results_4sources_state/slope_heatmap_Apr24_4sources.png", width = 8, height = 8, units = 'in', res = 300)
...
Rt_May3_EpiEstim_covidtracking <- as.numeric(as.character(unlist(lapply(state_Rt_EpiEstim_covidtracking,function(x){x[x$interval_end=="2020-04-24","mean_rt"]}))))
...
data_for_heatmap_May3 <- cbind(jhu = Rt_May3_EpiEstim_jhu,
                                covidtracking = Rt_May3_EpiEstim_covidtracking,
                                yu = Rt_May3_EpiEstim_yu)

M1 <- cor(data_for_heatmap_May3)
M1[which(M1 > 1)] <- 1
png("./COVID19-Rt/sensitivity_analysis/results_4sources_state/corr_heatmap_May3_4sources.png", width = 8, height = 8, units = 'in', res = 300)
corrplot(M1,
         method = "circle",
         tl.col = "black",
         type = "upper",
...
png("./COVID19-Rt/sensitivity_analysis/results_4sources_state/slope_heatmap_May3_4sources.png", width = 8, height = 8, units = 'in', res = 300)
...
Rt_stayathome_EpiEstim_covidtracking <- as.numeric(as.character(unlist(lapply(state_Rt_EpiEstim_covidtracking,function(x){x[as.character(x$interval_end)==as.character(lockdown[lockdown$State == x$state[1],"Date"]),"mean_rt"]}))))
...
data_for_heatmap_stayathome <- cbind(jhu = Rt_stayathome_EpiEstim_jhu,
                               covidtracking = Rt_stayathome_EpiEstim_covidtracking,
                               yu = Rt_stayathome_EpiEstim_yu)

M1 <- cor(data_for_heatmap_stayathome)
M1[which(M1 > 1)] <- 1
png("./COVID19-Rt/sensitivity_analysis/results_4sources_state/corr_heatmap_stayathome_4sources.png", width = 8, height = 8, units = 'in', res = 300)
corrplot(M1,
         method = "circle",
         tl.col = "black",
         type = "upper",
...
png("./COVID19-Rt/sensitivity_analysis/results_4sources_state/slope_heatmap_stayathome_4sources.png", width = 8, height = 8, units = 'in', res = 300)
