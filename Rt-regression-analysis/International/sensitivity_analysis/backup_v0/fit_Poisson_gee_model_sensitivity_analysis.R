...
# Analysis: Sensitivity analysis for country-level Rt regression
# Description: Poisson GEE direct fit of cases, offset(Lambda)
# Author: Hui Li, Derek Shyr, Zilin Li, Xihao Li
...
si_param <- data.frame(mu = c(5.2, 5.8, 7.5, 4.7, 3.96, 4.4),
                       sd = c(5.5, 4.5, 3.4, 2.9, 4.75, 3.0))

fit_no_mobility_coef <- matrix(NA, nrow = 31, ncol = (dim(si_param)[1])*3)
fit_mobility_coef <- matrix(NA, nrow = 32, ncol = (dim(si_param)[1])*3)

for (si_option in 1:(dim(si_param)[1])){
  
  print(paste0("mu=",si_param$mu,",sd=",si_param$sd)[si_option])
  ...
  dt[, Lam := getLambda(
    x = daily_inc,
    t = days_int, 
    mu = si_param$mu[si_option], sd = si_param$sd[si_option], NTS = 30
  ), by = countryName]
  ...
  fit_no_mobility_coef[,(si_option-1)*3+1:3] <- as.matrix(summary(fit_no_mobility)$coefficients[,c(1,2,4)])
  ...
  fit_mobility_coef[,(si_option-1)*3+1:3] <- as.matrix(summary(fit_mobility)$coefficients[,c(1,2,4)])
  
}


rownames(fit_no_mobility_coef) <- names(fit_no_mobility$coefficients)
colnames(fit_no_mobility_coef) <- 1:(dim(fit_no_mobility_coef)[2])
colnames(fit_no_mobility_coef)[(1:(dim(si_param)[1]))*3-2] <- paste0("beta(mu=",si_param$mu,",sd=",si_param$sd,")")
colnames(fit_no_mobility_coef)[(1:(dim(si_param)[1]))*3-1] <- paste0("se(mu=",si_param$mu,",sd=",si_param$sd,")")
colnames(fit_no_mobility_coef)[(1:(dim(si_param)[1]))*3] <- paste0("pvalue(mu=",si_param$mu,",sd=",si_param$sd,")")
fit_no_mobility_coef <- as.data.frame(fit_no_mobility_coef)


rownames(fit_mobility_coef) <- names(fit_mobility$coefficients)
colnames(fit_mobility_coef) <- 1:(dim(fit_mobility_coef)[2])
colnames(fit_mobility_coef)[(1:(dim(si_param)[1]))*3-2] <- paste0("beta(mu=",si_param$mu,",sd=",si_param$sd,")")
colnames(fit_mobility_coef)[(1:(dim(si_param)[1]))*3-1] <- paste0("se(mu=",si_param$mu,",sd=",si_param$sd,")")
colnames(fit_mobility_coef)[(1:(dim(si_param)[1]))*3] <- paste0("pvalue(mu=",si_param$mu,",sd=",si_param$sd,")")
fit_mobility_coef <- as.data.frame(fit_mobility_coef)


library(xlsx)
write.xlsx(fit_no_mobility_coef, file = "sensitivity_analysis/fit_Poisson_gee_results_sensitivity_analysis.xlsx",
           sheetName = "fit_no_mobility")

write.xlsx(fit_mobility_coef, file = "sensitivity_analysis/fit_Poisson_gee_results_sensitivity_analysis.xlsx",
           sheetName = "fit_mobility", append = TRUE)
